...
const collectMarkdown = async (directory) => {
    ...
    for (const item of await readdir(directory, { withFileTypes: true })) {
        ...
        if (item.isDirectory())
            found.push(...(await collectMarkdown(path)));
        else if (item.name.endsWith('.md'))
            found.push(path);
    }
    return found.sort();
};
const kindOf = (file) => file.includes('/roles/') ? 'role' : file.includes('/project-types/') ? 'project-type' : 'policy';
/**
 ...
 */
const deriveEntry = (file, content) => {
...
export { PACK_SUBDIRECTORY, collectMarkdown, deriveEntry, deriveRegistry, parseRegistry, resolveWithin, };
