...
import { GMAIL_ID, extractAttachments } from './mail.js';
/**
 ...
 */
const failure = (message) => new GraphError(message, false);
/**
 ...
 */
function toRgb(data, width, height, kind) {
    if (kind === 2)
        return data;
...
function downscale(rgb, width, height, factor) {
    if (factor <= 1)
 ...
 */
function resolveImage(page, id, ms = 5000) {
    return new Promise((resolve, reject) => {
        const timer = setTimeout(() => reject(new Error(`image ${id} did not decode within ${ms}ms`)), ms);
        try {
...
const boilerplateKey = (text) => text.toLowerCase().replace(/\d+/g, "#");
export async function extractPdfMap(bytes) {
    ...
    for (let n = 1; n <= pdf.numPages; n++) {
        ...
        const large = items.filter((i) => i.size >= median * HEADING_SIZE_RATIO &&
    ...
    const boilerplate = new Set([...appearances.entries()]
        .filter(([, count]) => count >= Math.max(2, Math.ceil(pdf.numPages / 2)))
        .map(([text]) => text));
    pages.forEach((entry, i) => {
        const heading = candidates[i].find((c) => !boilerplate.has(boilerplateKey(c.text)));
...
export async function extractPdfContent(bytes, fromPage = 1, pageCount) {
    const { getDocumentProxy } = await import("unpdf");
        ...
        for (let n = first; n <= last; n++) {
            ...
            if (images.length < imageBudget) {
                ...
                for (let i = 0; i < ops.fnArray.length; i++) {
                    ...
                    if (typeof id !== "string" || SHARED_IMAGE_ID.test(id))
                        continue;
                    let raw;
                    try {
                        ...
                        if (!unreadable.includes(n))
                            unreadable.push(n);
                        continue;
                    }
                    if (!raw?.data || raw.width * raw.height < MIN_IMAGE_PIXELS)
                        continue;
                    if (images.length >= imageBudget) {
            ...
            if (chars >= MAX_TEXT_CHARS || images.length >= imageBudget)
                break;
        }
    }
    finally {
...
export function describeGaps(extracted) {
    ...
    const list = (pages) => `${pages.length === 1 ? "Page" : "Pages"} ${pages.join(", ")}`;
    ...
    // sentence alone.
    const blind = extracted.pages_without_text.filter((p) => !shown.has(p) && p <= extracted.pages_searched_for_images);
    const recovered = extracted.pages_without_text.filter((p) => shown.has(p));
    const alongside = [...shown].filter((p) => !extracted.pages_without_text.includes(p));
    const notes = [
        ...
        !scanned && recovered.length > 0
            ? `${list(recovered)} carried little or no text and ${recovered.length === 1 ? "is" : "are"} attached below as ${recovered.length === 1 ? "a picture" : "pictures"}` +
        ...
        bigScan
            ? `This is a ${extracted.pages_total}-page document of page images. ` +
                `Reading all of it would be roughly ${Math.round(extracted.pages_total * TOKENS_PER_PAGE_IMAGE / 1000)}k tokens of pictures, which is not practical here, so do not page ` +
...
const isDocx = (mimeType, filename) => mimeType.toLowerCase() === DOCX_TYPE || /\.docx$/i.test(filename);
...
export function imageSize(bytes) {
    ...
    if (bytes[0] === 0xff && bytes[1] === 0xd8) {
        ...
        while (i < bytes.length - 9) {
            ...
            if (length < 2)
                return null; // malformed; refuse to loop on it
            i += 2 + length;
        }
        return null;
    }
 ...
 */
export function imageResult(meta, bytes) {
    const base = {
...
export function unsupportedNote(mimeType, filename) {
...
async function loadAttachment(token, emailId, attachmentId) {
    ...
    const attRes = await gmailGet(`/users/me/messages/${id}/attachments/${encodeURIComponent(meta.gmail_id)}`, token);
 ...
 */
export async function mapAttachment(token, emailId, attachmentId) {
    const loaded = await loadAttachment(token, emailId, attachmentId);
    if (loaded.oversized)
        return tooLargeResult(loaded.meta);
    ...
    try {
        ...
        return {
            ...
            kind: "unreadable",
            pages: [],
            note: "This PDF could not be parsed. It may be encrypted, password " +
                "protected, or damaged.",
        };
    }
    return {
        ...base,
        kind: map.scanned ? "scan" : "text",
...
export async function readAttachment(token, emailId, attachmentId, fromPage, pageCount) {
    if (typeof emailId !== "string" || !GMAIL_ID.test(emailId)) {
        throw failure("email_id is missing or malformed.");
    }
    if (typeof attachmentId !== "string" || !ATTACHMENT_ID.test(attachmentId)) {
    ...
    if (mime.startsWith("image/"))
        return imageResult(meta, bytes);
    if (isDocx(meta.mime_type, meta.filename)) {
        let doc;
        try {
            doc = await extractDocxContent(bytes, fromPage ?? 1);
        }
        catch (err) {
    ...
    let extracted;
    try {
        extracted = await extractPdfContent(bytes, fromPage ?? 1, pageCount);
    }
    catch (err) {
...
async function unzipEntry(bytes, wanted) {
    ...
    for (let n = 0; n < count; n++) {
        ...
        if (name === wanted) {
            ...
            if (method === 0)
                return data; // stored rather than deflated
            if (method !== 8)
                return null; // anything else is not worth supporting
            const inflated = await new Response(new Blob([data]).stream().pipeThrough(new DecompressionStream("deflate-raw"))).arrayBuffer();
 ...
 */
export async function extractDocxContent(bytes, fromPart = 1) {
