...
import { clearProvider, loadTokens, readProvider } from './tokens.js';
const PROVIDER_NAMES = Object.keys(PROVIDERS);
const parseProvider = (input) => {
    if (input === undefined)
        return 'microsoft';
    if (PROVIDER_NAMES.includes(input))
        return input;
...
export const runConnect = async (input) => {
 ...
 * not.
 */
export const runDisconnect = async (input) => {
    const provider = parseProvider(input);
    ...
    console.log(`The grant itself still exists in your ${config.label} account. To withdraw it ` +
