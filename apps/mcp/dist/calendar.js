...
export function eventTime(t) {
    if (!t)
        return { value: null, all_day: false, time_zone: null };
    if (t.date)
        return { value: t.date, all_day: true, time_zone: t.timeZone ?? null };
 ...
 */
export function thinRecurring(events, limit) {
    const seen = new Map();
    const kept = [];
    let omitted = 0;
 ...
 */
export async function listEvents(token, rawCalendarId, rawQuery, rawTimeMin, rawTimeMax) {
    ...
    const res = await calendarGet(`/calendars/${encodeURIComponent(calendarId)}/events?${params}`, token);
...
export async function readEvent(token, rawEventId, rawCalendarId) {
    ...
    const res = await calendarGet(`/calendars/${encodeURIComponent(calendarId)}/events/${encodeURIComponent(rawEventId)}`, token);
