 ...
 */
const fetchGoogleClientSecret = async () => {
    let res;
    try {
    ...
    if (!res.ok || typeof body.google_client_secret !== 'string') {
        throw new GraphError(`Could not get the Google client configuration: ${body.error ?? `HTTP ${res.status}`}`, false);
    }
...
export const PROVIDERS = {
    ...
    google: {
        ...
        clientId: env('ARTIST_MCP_GOOGLE_CLIENT_ID', '993381632576-33o752gthtkehtcbhoe2djssi1kepm03.apps.googleusercontent.com'),
 ...
 * whether it worked.
 */
const awaitCode = (expectedState) => new Promise((resolve, reject) => {
    ...
    server.on('error', (err) => {
        reject(err.code === 'EADDRINUSE'
            ? new Error(`Port ${REDIRECT_PORT} is already in use, and sign-in has to use exactly ` +
 ...
 */
const postToken = async (config, form, clientSecret) => {
    const body = { client_id: config.clientId, ...form };
    ...
    if (!res.ok) {
        ...
        throw new GraphError(`${config.label} rejected the sign-in: ${parsed.error_description ?? parsed.error ?? res.status}` +
            (reconnect ? '. Reconnect needed.' : ''), reconnect);
    }
    return parsed;
};
/**
...
export const connect = async (provider) => {
    ...
    if (tokens.refresh_token === undefined) {
        throw new Error(`${config.label} returned no refresh token, so this connection could not ` +
 ...
 */
export const accessTokenFor = async (provider) => {
    const config = PROVIDERS[provider];
