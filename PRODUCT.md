...
- No writes to any source apart from the one opt-in below: no OneNote edits, message sending, replying, drafting, labelling, calendar editing or deletion, RSVP responses, background jobs, synchronization, or workflow state in Supabase.
- One exception, opt-in per install and off by default: creating a single Google Calendar event, previewed and confirmed. A gig that lives on a page and never reaches the calendar makes the musician the sync layer, which is the thing this product exists to stop being true. It went first because a wrong calendar event is deleted in two seconds and a corrupted OneNote page is not. See [docs/decisions/0001-opt-in-calendar-writes.md](docs/decisions/0001-opt-in-calendar-writes.md).
